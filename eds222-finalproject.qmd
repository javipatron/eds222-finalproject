...
mangrove_2017 |> 
  ...
  geom_point(aes(y= mean(total_tree_mg_c_ha)), size = 10, alpha = 0.9, color = "darkblue", shape = 3 ) +
  geom_point(alpha = 0.7) +
...
alpha = 0.05
power = 0.90
random_d = as.numeric(sqrt(random_sd_25m^2/300 + random_sd_154m^2/300))
print(random_d)
...
s1 = power_table_test[[1,9]]
s2 =  power_table_test[[3,9]]

alpha = 0.05
power = 0.90
random_d = as.numeric(sqrt(s1^2/samples + s2^2/samples))

pwr.t.test(n =samples, 
           d = random_d, 
           sig.level=.05, 
           type="one.sample",
           alternative = "greater")
