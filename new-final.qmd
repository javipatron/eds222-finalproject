...
**Alternative Hypothesis-** The Total Carbon per Tree (mg) in the sample size of 25m2 is different than the one to the 154m2.
...
**The Null Hypothesis -** The Total Carbon per Tree (mg) mean in the Sample Size of 77m2 is no different to the 154m2.

**Alternative Hypothesis-** The Total Carbon per Tree (mg) in the sample size of 77m2 is different than the one to the 154m2.
...
ggplot(plots_2017, aes(x= sample_count, y = sd_tree_c_ha)) +
  ...
  labs(title = "Tree Carbon variance per Sample plot",
       x = "Samples per Plot",
       y = "Variance of Tree carbon (mg/ha)") +
  geom_hline(yintercept = 0.05, color = "gray10")
```
