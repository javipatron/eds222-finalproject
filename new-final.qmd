...
**Option 2:** What is the density difference between sample sizes.
...
```
**The standard error is the observed difference in Tree Carbon (mg/ha) which is 20.2 standard deviations above the null of "zero difference".**
...
We are 95% confident that the true value lies between 0.308 - 0.374 Tree Carbon sequestration in mg per hectare.
...
Calculate the manual p-value
```{r}
pval_2 = 2* pnorm(point_est_2, mean = 0, sd = SE_2, lower.tail = FALSE)
...
Create a plot graph for the density
```{r}

ggplot(density_df, aes(x= as.factor(plot_size_m2), y= density, col = as.factor(plot_size_m2))) +
  geom_boxplot() +
